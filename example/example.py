...
with sch.ICBuilder(s, 32, pin_len=300) as ic:
    ...
    for i in range(4):
        ic.pin(numbers=6+i, name='PWM2_{0}'.format(i+1), type=sch.Pin.Bidirectional)
    ic.pin(numbers=10, name='PWM14', type=sch.Pin.Bidirectional)
    for i in range(4):
        ic.pin(numbers=12+i, name='PWM3_{0}'.format(i+1), type=sch.Pin.Bidirectional)
    for i in range(3):
        ic.pin(numbers=18+i, name='PWM3_{0}'.format(i+1), type=sch.Pin.Bidirectional)
l.devices.append(
    kidraw.Device(
        schematic=s,
        footprints=[
